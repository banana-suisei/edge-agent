...
and the underlying store operations (InMemoryStore and PostgresStore)."""
...
class TestPostgresStore:
    ...
    def cleanup_test_data(self, pg_config):
        ...
        for ns in [("test",), ("users",), ("ns1",), ("ns2",), ("prefs",), ("notes",)]:
...
class TestMemoryToolsInMemory:
    ...
    def test_save_and_get(self, rt, store):
        result = self._save("users/u1", "name", "Alice", rt)
        assert "Saved" in result
        item = store.get(("users", "u1"), "name")
        assert item.value == {"content": "Alice"}
    ...
    def test_search_returns_results(self, rt, store):
        ...
        result = self._search("notes", "Buy", rt)
        assert "n1" in result
        assert "n2" in result
...
# ===========================================================================
# Memory-tools integration tests (PostgresStore)
# ===========================================================================
...
class TestMemoryToolsPostgres:
    ...
    def cleanup(self, pg_config):
        yield
        store = create_store(pg_config)
        for ns in [("users", "u1"), ("prefs",), ("notes",), ("test",)]:
            items = store.search(ns)
            for item in items:
                store.delete(ns, item.key)
    ...
    def test_search_finds_items(self, rt, store):
        store.put(("notes",), "n1", {"content": "meeting at 3pm"})
        store.put(("notes",), "n2", {"content": "call mom"})
        result = search_memory.func(namespace="notes", query="meeting", runtime=rt)
        assert "n1" in result
    ...
    def test_full_lifecycle(self, rt, store):
        ...
        found = search_memory.func(namespace="users/u1", query="Rust", runtime=rt)
